...
def main(args):
    ...
    if not os.path.exists(checkpoints_path + '/nic'):
        os.mkdir(checkpoints_path + '/nic')

    path_checkpoint = checkpoints_path + (
        '/nic/{}.checkpoint.id.injection{}.layer{}.state{}.embedding{}.keras'
    ).format(dataset, injection_mode, lstm_layers, state_size, embedding_size)
    ...
    log_dir = (logs_path + '/nic/{mode}/{injection}/'
    ...
    if load_model == 1:
        try:
            nic.load(path_checkpoint)
        except Exception as error:
            print("Error trying to load checkpoint.")
            print(error)

    nic.model.fit_generator(
    ...
    scores = nic.model.evaluate_generator(
