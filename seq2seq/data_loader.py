...
class FlickrStyleDataset(data.Dataset):
    ...
    def __init__(self,
                 ...
                 transform=None):
        self.img_dir = img_dir
        self.imgname_caption_list = self._get_imgname_and_caption(
            caption_file, style_caption_file)
        self.vocab = vocab
        self.transform = transform

    def _get_imgname_and_caption(self, caption_file, style_caption_file):
        with open(caption_file, 'r') as f:
            res = f.readlines()
        ...
        for line in res:
            img_and_cap = r.split(line)
            img_and_cap = [x.strip() for x in img_and_cap]
            imgname = img_and_cap[0]
            cap = img_and_cap[-1]
            if imgname_caption_map.get(imgname) is None:
                imgname_caption_map[imgname] = []
            imgname_caption_map[imgname].append(cap)

        # style caption
        with open(style_caption_file, 'r') as f:
            res = f.readlines()
        imgname_caption_list = []
        r = re.compile(r'#\d*')
        for line in res:
            ...
            img_and_cap = [x.strip() for x in img_and_cap]
            imgname = img_and_cap[0]
            for factual_cap in imgname_caption_map[imgname]:
                imgname_caption_list.append(img_and_cap + [factual_cap])
    ...
    def __getitem__(self, index):
        ...
        img_name = os.path.join(self.img_dir, img_name)
        caption = self.imgname_caption_list[index][1]
        caption_factual = self.imgname_caption_list[index][2]

        image = FlickrStyleDataset.__cache.get(img_name)
        if image is None:
            image = Image.open(img_name).convert('RGB')
            if self.transform is not None:
                image = self.transform(image)
            FlickrStyleDataset.__cache[img_name] = image
        ...
        # Convert caption (string) to word ids.
        tokens = nltk.tokenize.word_tokenize(str(caption_factual).lower())
        caption = []
        caption.append(self.vocab('<start>'))
        caption.extend([self.vocab(token) for token in tokens])
        caption.append(self.vocab('<end>'))
        source = torch.Tensor(caption)
...
def collate_fn_styled(data):
    # Sort a data list by caption length (descending order).
    data.sort(key=lambda x: len(x[1]), reverse=True)
    images, factual, captions = zip(*data)

    # Merge images (from tuple of 3D tensor to 4D tensor).
    images = torch.stack(images, 0)

    # Merge captions (from tuple of 1D tensor to 2D tensor).
    lengths = [len(cap) for cap in factual]
    targets = torch.zeros(len(factual), max(lengths)).long()
    for i, cap in enumerate(factual):
        end = lengths[i]
        targets[i, :end] = cap[:end]
    src = (targets, lengths)
...
def get_style_loader(img_dir, caption_file, style_caption_file, vocab,
    ...
    flickr_style = FlickrStyleDataset(img_dir=img_dir,
                                      caption_file=caption_file,
                                      style_caption_file=style_caption_file,
                                      vocab=vocab,
                                      transform=transform)
