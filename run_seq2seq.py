...
from model import Seq2Seq, StyleNet
from preprocess.tokenizer import mark_captions
from utils.generator import seq2seq_batch_generator
from utils.evaluator import bleu_evaluator
from utils.predictor import generate_caption, seq2seq_generate_caption
...
def main(args):
    ...
    with_transfer_value = 1  # always use transfer value on seq2seq
    ...
    print('set encoder weights from factual stylent. OK')
    ...
    seq2seq.save(path_checkpoint, True)
    ...
    for filename, refs in zip(filenames_test, captions_test):
        output_tokens, transfer_values, output_text = generate_caption(
            image_path=dataset_path + '/img/' + filename,
            tokenizer=tokenizer,
            rich_model=stylenet,
        )
        _, _, output_text = seq2seq_generate_caption(
