...
class RNNModel:
    ...
    def __init__(self, conf_new=None, encoder=None):
        ...
        conf['param_names_saveable'] = ["wemb", "h0_hidden",
                                        "h0_cell", "wvm", "bmv", "w", "b", "w_lstm"]
        if conf['BATCH_NORM']:
    ...
    def save_model(self, filename, to_file=True):
        ...
        if self.conf['DECODER']:
            ...
            pickle.dump((self.conf, self.model, encoder_data),
                        open(filename, "wb"), protocol=2)
        else:
            if to_file:
                pickle.dump((self.conf, self.model),
                            open(filename, "wb"), protocol=2)
            else:
    ...
    def build_shared_layers(self, layers):
        ...
        for name, data in list(layers.items()):
            setattr(self, name, theano.shared(
                name=name+ne, value=data, borrow=True))
    ...
    def load_model(self, filename, conf=None, load_solver_params=True):
        ...
        if len(data) == 3:
            ...
            self.encoder.load_model_from_data(
                encoder_data, conf, load_solver_params)
        else:
            self.load_model_from_data(data, conf, load_solver_params)
        ...
        if load_solver_params:
            ...
            for i, p in enumerate(self.conf['param_names_trainable']):
                if p in self.model['hist_grad']:
                    ...
                    self.hist_grad.append(theano.shared(
                        name="hist_grad[%d]" % i, value=v, borrow=True))
                else:
                ...
                if p in self.model['delta_grad']:
                    ...
                    self.delta_grad.append(theano.shared(
                        name="delta_grad[%d]" % i, value=v, borrow=True))
                else:
                    v = np.zeros_like(getattr(self, p).get_value(borrow=True))
                    self.delta_grad.append(theano.shared(
                        name="delta_grad[%d]" % i, value=v, borrow=True))
    ...
    def make_new_train_drop_masks(self):
        ...
        new_X_drop = make_drop_mask(
            num_drop_masks, self.conf['MAX_SENTENCE_LEN']+1, self.conf['emb_size'], self.conf['DROP_INPUT_FRACTION'])
        new_Y_drop = make_drop_mask(
            num_drop_masks, self.conf['MAX_SENTENCE_LEN']+1, self.conf['lstm_hidden_size'], self.conf['DROP_OUTPUT_FRACTION'])
    ...
    def setup_dataprovider(self, load_vocab=True):
        if not hasattr(self, 'dp'):
            self.dp = RNNDataProvider(
                self.conf['DATASET'], self.conf['REVERSE_TEXT'])
        if not self.dp.loaded:
            ...
            if not load_vocab:
                ...
                self.dp.build_vocab(
                    self.conf['MIN_WORD_FREQ'], source=self.conf['ENCODE_EXTRA'])
    ...
    def load_training_dataset(self, rotate=False):
        ...
        self.X_train, self.Xlen_train, self.V_train, _ = self.dp.get_data_split(
            data_split=self.conf['TRAIN_SPLIT'], randomize=True, pad_len=self.conf['MAX_SENTENCE_LEN'], rotate_X_with_id=rotate, source=self.conf['ENCODE_EXTRA'])
        self.X_train_mask = build_len_masks(
            self.Xlen_train+1, self.conf['MAX_SENTENCE_LEN']+1)
    ...
    def load_val_dataset(self, rotate=False):
        ...
        self.X_valid_mask = build_len_masks(
            self.Xlen_valid+1, self.conf['MAX_SENTENCE_LEN']+1)
...
#                            on_unused_input='ignore')
        ...
        if self.conf['JOINED_MODEL']:
            ...
            if self.conf['DECODER']:
                ...
                givens = {self.x_drop: mX[:self.x.shape[0]],
                          ...
                          self.forced_word: T.ones((self.x.shape[1]+1, self.x.shape[0]), dtype=np.int32)}
            self.get_ppl_val = theano.function(
                [self.x, self.v, self.xlen, self.encoder.x,
                    self.encoder.v, self.encoder.xlen],
                outputs=[self.perplexity_batch_v, self.perplexity_batch_n],
                givens=givens,
                on_unused_input='ignore')
            # self.get_ppl_sent_val = theano.function(
    ...
    def build_sentence_generator(self):
        self.gen_sentence = theano.function(
            [self.v_single, self.nstep], outputs=self.wout_fb)
        ...
        while True:
            ...
            if cur_idx == 0 and num_iter != 0:
                ...
                if self.conf['DECODER']:
                    ...
                    self.X_sh_train_mask.set_value(
                        self.X_sh_train_mask.get_value(borrow=True)[idx], borrow=True)
                    self.encoder.X_sh_train.set_value(
                        self.encoder.X_sh_train.get_value(borrow=True)[idx], borrow=True)
                    self.encoder.V_sh_train.set_value(
                        self.encoder.V_sh_train.get_value(borrow=True)[idx], borrow=True)
                    self.encoder.X_sh_train_mask.set_value(
                        self.encoder.X_sh_train_mask.get_value(borrow=True)[idx], borrow=True)
                ...
                if self.conf['DECODER']:
                    ppl_v, ppl_n = self.get_ppl_val(self.X_valid[ii:ii+batch_size_val],
                                                    ...
                                                    self.X_valid_mask[ii:ii +
                                                                      batch_size_val],
                                                    self.encoder.X_valid[ii:ii +
                                                                         batch_size_val],
                                                    self.encoder.V_valid[ii:ii +
                                                                         batch_size_val],
    ...
    def do_one_step(self, v_i, last_step=None, encoder_words=None):
        if last_step is not None:
            ...
            if self.conf['DECODER']:
                hh = self.encoder.start_step(np.reshape(
                    encoder_words, (1, -1)), np.reshape(v_i, (1, -1)))[0]
                #v_i = hh
                v_i = np.concatenate([hh, v_i], axis=1)

        hh, cc, s_t = self.one_step(
            step['word_t'], step['use_v'], step['h_hid'], step['h_cell'], v_i)
