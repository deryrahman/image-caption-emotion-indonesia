...
class AttentionDecoder(Recurrent):
    ...
    def build(self, input_shapes):
        ...
        if isinstance(input_shapes[0], (list, tuple)):
            ...
            self.input_spec = [
                InputSpec(shape=input_shape) for input_shape in input_shapes
            ]
        ...
        self.W_r = self.add_weight(
            shape=(self.embedding_dim, self.units),
            name='W_r',
            initializer=self.kernel_initializer,
            regularizer=self.kernel_regularizer,
            constraint=self.kernel_constraint)
        ...
        self.W_o = self.add_weight(
            shape=(self.embedding_dim, self.output_dim),
            name='W_o',
            initializer=self.kernel_initializer,
            regularizer=self.kernel_regularizer,
            constraint=self.kernel_constraint)
        ...
        if self.is_monotonic:
            self.Energy_r = self.add_scalar(initial_value=-1, name='r')
            self.states.append(None)
        if self.normalize_energy:
            self.Energy_g = self.add_scalar(initial_value=1, name='g')
    ...
    def get_initial_state(self, inputs):
        ...
        if self.is_monotonic:
            ...
            alpha0 = K.switch(
                K.greater(memory_shape[1], 1), lambda: K.concatenate(
                    [alpha0,
                     K.zeros((memory_shape[0], memory_shape[1] - 1))],
                    axis=-1), alpha0)
    ...
    def _compute_energy(self, stm):
        ...
        et = K.dot(
            activations.tanh(K.expand_dims(_stm, axis=1) + self._uxpb),
            K.expand_dims(V_a))
    ...
    def _compute_probabilities(self, energy, previous_attention=None):
        if self.is_monotonic:
            ...
            noise = K.random_normal(
                K.shape(energy), mean=0.0, stddev=sigmoid_noise)
            # encourage discreteness in train
            energy = K.in_train_phase(energy + noise, energy)

            p = K.in_train_phase(
                K.sigmoid(energy), K.cast(energy > 0, energy.dtype))
            p = K.squeeze(p, -1)
