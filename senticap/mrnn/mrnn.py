...
class RNNModel:
    ...
    def build_shared_layers(self, layers):
        for name, data in list(layers.items()):
            setattr(self, name, theano.shared(
                name=name, value=data, borrow=True))
    ...
    def load_training_dataset(self):
        ...
        self.X_train, self.Xlen_train, self.V_train, _ = self.dp.get_data_split(
            data_split=self.conf['TRAIN_SPLIT'], randomize=True, pad_len=self.conf['MAX_SENTENCE_LEN'])
    ...
    def build_model_trainer(self):
        ...
        comp_grads = [g*m if m is not None else g for g,
    ...
    def build_perplexity_calculator(self):
        ...
        if self.conf['JOINED_MODEL']:
            ...
            self.get_ppl_val = theano.function(
                ...
                outputs=[self.perplexity_batch_v,
                         self.perplexity_batch_n],
                givens=givens,
                on_unused_input='ignore')
